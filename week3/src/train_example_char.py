...
# Load data
data_train = Data(train_df, partitions['train'], img_path=img_path, tokenizer=char_tokenizer)
data_valid = Data(val_df, partitions['val'], img_path=img_path, tokenizer=char_tokenizer)
data_test = Data(test_df, partitions['test'], img_path=img_path, tokenizer=char_tokenizer)
...
# Create the model
model = Model(num_char=len(char_tokenizer), char2idx=char2idx, text_max_len=TEXT_MAX_LEN).to(DEVICE)
