...
class FasterRCNN:
    ...
    def evaluate_model(self, data_dir: str, dataset_name: str = "kitti-mots", num_classes: int = 2, output_dir: str = "./output/eval") -> dict:
        ...
        if not self.is_coco:
            ...
            MetadataCatalog.get(dataset_name).set(thing_classes=["0"])
            self.cfg.MODEL.ROI_HEADS.NUM_CLASSES = num_classes
        else:
    ...
    def train_model(self, data_dir: str, dataset_name: str = "kitti-mots", num_classes: int = 2, output_dir: str = "./output/train", **kwargs) -> dict:
        ...
        try:
            for split in ["train", "val"]:
                DatasetCatalog.register(dataset_name + "_train", lambda: get_YOLOData_dicts(dataset_name+"/train"))
                DatasetCatalog.register(dataset_name + "_val", lambda: get_YOLOData_dicts(dataset_name+"/valid"))
        ...
        self.cfg.SOLVER.BASE_LR = 0.00316021059379249  # Learning rate
        self.cfg.SOLVER.LR_SCHEDULER_NAME = "WarmupMultiStepLR"
        self.cfg.SOLVER.MAX_ITER = 3000    # Maximum iterations
        #self.cfg.SOLVER.STEPS = (3000, 4500)  # Learning rate decay steps
        #self.cfg.SOLVER.GAMMA = 0.1        # Learning rate decay factor
        self.cfg.SOLVER.WEIGHT_DECAY = 0.0004056016180634831
        self.cfg.SOLVER.CHECKPOINT_PERIOD = 1000  # Save checkpoint every 1000 iterations
        self.cfg.MODEL.ROI_HEADS.BATCH_SIZE_PER_IMAGE = 256  # Faster, and good enough for COCO pre-trained models
        ...
        #val_loader = build_detection_test_loader(eval_cfg, dataset_name + "_test")
...
if __name__ == "__main__":
    ...
    if task == "train":
        model = FasterRCNN("COCO-Detection/faster_rcnn_R_50_FPN_3x.yaml", "COCO-Detection/faster_rcnn_R_50_FPN_3x.yaml", num_frozen_layers=0)
        print("Start Training")
        results = model.train_model("/ghome/c5mcv01/mcv-c5-team1/data", dataset_name= "AquariumDataCots", num_classes=1, output_dir="./output/GlobalWheat/training/trainF5_opt0_3000iter")
        print("Results: ", results)

    elif task == 'infer':
        model_weigths = "/ghome/c5mcv01/mcv-c5-team1/week1/src/domain_shift/output/Aquarium/..."
        model = FasterRCNN("COCO-Detection/faster_rcnn_R_50_FPN_3x.yaml", model_weights, num_frozen_layers=0)
        #input_image = "/ghome/c5mcv01/mcv-c5-team1/week1/src/domain_shift/GlobalWheat/test/uq_1/063b1190-8045-4b6b-ae85-731475d82ca0.png"
        #input_image = "/ghome/c5mcv01/mcv-c5-team1/week1/src/domain_shift/GlobalWheat/test/utokyo_1/0bd71b05-0f8c-43f7-84c5-6033b0885141.png"
        #input_image = "/ghome/c5mcv01/mcv-c5-team1/week1/src/domain_shift/GlobalWheat/test/nau_1/32aa6056-9afd-4ef7-bc02-1defb76c20a6.png"
        #input_image = "/ghome/c5mcv01/mcv-c5-team1/week1/src/domain_shift/GlobalWheat/test/utokyo_2/143ad1ee-1728-4154-aeae-14d10bc04c2c.png"
        input_image = "/ghome/c5mcv01/mcv-c5-team1/week1/src/domain_shift/GlobalWheat/test/utokyo_1/0a8b589a-a972-43cc-87e0-91d4afe7fe71.png"
        ...
        cv2.imwrite(f"{output_dir}/INFER_F2_visualized_image_finetuned5.png", visualized_image)
'''
...
if __name__ == "__main__":
    ...
    if task == "train":
        ...
        print("Start Training")
        results = model.train_model("/ghome/c5mcv01/mcv-c5-team1/data", dataset_name= "GlobalWheat", num_classes=1, output_dir="./output/GlobalWheat/training/ppt/trainF5_opt0_3000iter")
        print("Results: ", results)

    elif task == "test":
        model_weights = "/ghome/c5mcv01/mcv-c5-team1/week1/src/domain_shift/output/GlobalWheat/training/trainF0__opt0/model_final.pth"
        model = FasterRCNN("COCO-Detection/faster_rcnn_R_50_FPN_3x.yaml", model_weights, num_frozen_layers=0)
        print("Start Training")
        results = model.train_model("/ghome/c5mcv01/mcv-c5-team1/data", dataset_name= "GlobalWheat", num_classes=1, output_dir="./output/GlobalWheat/training/test_F0_opt0")
        print("Results: ", results)

    elif task == 'infer':
        model_weights = "/ghome/c5mcv01/mcv-c5-team1/week1/src/domain_shift/output/GlobalWheat/training/trainF2/model_final.pth"
        model = FasterRCNN("COCO-Detection/faster_rcnn_R_50_FPN_3x.yaml", model_weights, num_frozen_layers=0)
        #input_image = "/ghome/c5mcv01/mcv-c5-team1/week1/src/domain_shift/GlobalWheat/test/uq_1/063b1190-8045-4b6b-ae85-731475d82ca0.png"
        #input_image = "/ghome/c5mcv01/mcv-c5-team1/week1/src/domain_shift/GlobalWheat/test/utokyo_1/0bd71b05-0f8c-43f7-84c5-6033b0885141.png"
        #input_image = "/ghome/c5mcv01/mcv-c5-team1/week1/src/domain_shift/GlobalWheat/test/nau_1/32aa6056-9afd-4ef7-bc02-1defb76c20a6.png"
